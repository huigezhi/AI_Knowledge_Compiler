...
5. 想顺便让 AI 提炼知识，点「**保存 + 编译**」（需要先配模型 API Key，见下）

其它平台同理：ChatGPT、Claude、豆包、智谱清言，打开会话 → 点图标 → 保存。

**其实第 4 步不用手动点**——见下面「自动保存」。

---

## 自动保存：以后不用再点按钮了

默认已开启。你在这个平台聊天时，内容一变，扩展会等你**停手 15 秒**后自动归档；
AI 正在流式输出时不会打断你。侧边栏顶部会显示：

```
自动保存   开启中 · 上次保存 2 分钟前（长川科技）
```

- 想改等待时长 / 关闭：扩展「设置」页 → 采集策略 → **自动保存当前会话**。
- 关闭后仍可点「保存当前」手动保存。

## 自动同步历史：没点开过的会话也能采到

以前「批量同步」要求你**手动逐个打开**每个会话才能采——因为没打开的会话在页面上根本不存在。
现在不用了：侧边栏点「**自动同步历史**」，扩展会：

1. 读取历史列表；
2. **在这个标签页里依次打开每个会话**、等页面渲染好、采集、入库；
3. 全部跑完后**自动回到你原来的页面**。

进度实时显示在侧边栏（`同步中：3/20 · 成功 3 · 失败 0`），随时可以点「取消」（也会立刻回到原页面）。

> 注意：期间这个标签页会被占用，**不要操作它**；想继续聊天就另开一个标签页。
> 默认会跳过**已经采集过**的会话，所以第二次跑只会补新的。
> 极少数历史项取不到链接（如某些平台的分组标题）会被跳过，这类仍需手动打开一次。
